- [RiseGOSEKIcapture](#risegosekicapture)
  - [使い方](#使い方)
    - [GOSEKIcapture2.exeの起動](#gosekicapture2exeの起動)
    - [キャプチャシーンに合わせる](#キャプチャシーンに合わせる)
    - [スライドバーを移動させてカットオフ値を決める](#スライドバーを移動させてカットオフ値を決める)
    - [mesbox(テキスト画面)のOnOff](#mesboxテキスト画面のonoff)
    - [コピペ](#コピペ)
  - [注意事項](#注意事項)
    - [文字化け(ver1でもver2でも)](#文字化けver1でもver2でも)
    - [UIがはみ出てる](#uiがはみ出てる)
    - [装飾品がついている場合](#装飾品がついている場合)
    - [OpenCLデバイスがない](#openclデバイスがない)
    - [計算に使うGPU,CPUを手動で指定したい](#計算に使うgpucpuを手動で指定したい)
  - [スキル文字認識の原理](#スキル文字認識の原理)
  - [更新履歴](#更新履歴)
- [GOSEKIcapture2_CUI](#gosekicapture2_cui)
  - [対応形式](#対応形式)
  - [実行](#実行)
  - [結果出力](#結果出力)
  - [デバッグ出力](#デバッグ出力)
...
### 文字化け(ver1でもver2でも)
 ...
 これは**表示だけの問題**であり、ソフトの内部では正しい文字列で記憶されています。**Escキー**を押してソフトを終了することにより正しい文字列が「テキスト出力.txt」という名前で保存されます。  
 
### UIがはみ出てる
 ![juhuku](https://user-images.githubusercontent.com/44022497/116908342-bc00f180-ac7d-11eb-8df5-df91b7788260.png)  
 これは正しく表示されているときですが、「重複を数える」というチェックボックスや矢印のボタンが正しく表示されない場合があります。  
...
![mhr](https://user-images.githubusercontent.com/44022497/118169366-d1d39b00-b463-11eb-8558-dd0332087345.jpg)  
 ...
 もし装備画面のシーンなら下記のよう切り取り位置の設定を変えてください。  
